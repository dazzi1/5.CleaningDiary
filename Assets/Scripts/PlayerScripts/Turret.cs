    ...
    void Update()
    {
        ...
        if (isEnemyTurret)//敌人炮塔
        {
            ...
            if (hit.collider != null)
            {
                if (!hit.collider.gameObject.CompareTag("Wall"))
                {
                    ...
                    {
                        Instantiate(bulletGo, shootPoints[0].position, Quaternion.Euler(0, 0,
        ...
        else//玩家炮塔
        {
            for (int i = 0; i < player.enemyList.Count; i++)
            {
                //防止射线自我检测
                hit = Physics2D.Raycast(transform.position,
                    player.enemyList[i].transform.position - transform.position, 5, layerMask);
                if (hit.collider != null)
                {
                    if (!hit.collider.gameObject.CompareTag("Wall"))
                    {
            ...
            if (nearestEnemy != null)
            {
                ...
                if (curAttackCD <= 0 && bullets > 0)
                {
                    ...
                    if (turretLevel >= 3)
                    {
                        Instantiate(bulletGo, shootPoints[1].position, Quaternion.Euler(0, 0,
                        transform.rotation.eulerAngles.z + Random.Range(-inaccuracy, inaccuracy)));
                    }
                    audioSource.PlayOneShot(shootClip, GameManager.Instance.volume * 0.6f);
                    curAttackCD = attackCD;
