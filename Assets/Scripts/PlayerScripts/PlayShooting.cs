...
public class PlayShooting : MonoBehaviour
{
    ...
    public double aimingDeSpeed;//瞄准不精度减少速度
    ...
    //当前属性
    private double curInaccuracy;//当前不精确度
    private double curDestabilization;//当前不稳定性
    private double curAttackCD;//攻击CD
    public int curMagazine;//当前弹夹里的子弹数量
    public int curBullets;//当前全部的子弹数量(背包+弹夹)
    public double curReload;//装弹CD
    ...
    void Start()
    {
        ...
        maxInaccuracy = weaponProperties.maxInaccuracy;
        recoilForce = weaponProperties.recoilForce;
        destabilization = weaponProperties.destabilization;
        aimingDeSpeed = weaponProperties.aimingDeSpeed;
        minInaccuracy = weaponProperties.minInaccuracy;
        attackCD = weaponProperties.attackCD;
        magazine = weaponProperties.magazine;
        totalBullets = weaponProperties.totalBullets;
        reload = weaponProperties.reload;
        weight = weaponProperties.weight;
        repulsion = weaponProperties.repulsion;
        audioSource = GetComponent<AudioSource>();
        curBullets = totalBullets;
        curMagazine = magazine;
        curReload = curAttackCD = 0.0;
    ...
    private void CalcultateCD()
    {
        ...
        GameManager.Instance.gameUIManager.UpdateAttackCDUI((float)((attackCD-curAttackCD)/attackCD));

        if (curReload > 0)
        {
            //装弹中
            curReload -= Time.deltaTime;
            GameManager.Instance.gameUIManager.UpdateReloadAndMagazineBulletUI(
                (float)((reload - curReload) / reload));
        }
        else
        {
            if (curMagazine <= 0)
    ...
    private void CalculateDestabilization()
    {
        if (player.isMoving)
        {
            ...
            {
                curDestabilization += destabilization * Time.deltaTime * 1.5;
            }
            else
            {
    ...
    private void CalculateInaccuracy()
    {
        ...
        lineLeft.transform.localRotation = Quaternion.AngleAxis((float)curInaccuracy, Vector3.forward);
        lineRight.transform.localRotation = Quaternion.AngleAxis((float)-curInaccuracy, Vector3.forward);
    }
    /// <summary>
    /// 攻击射击
    /// </summary>
    ...
    private void CreateBullet(int bulletIndex)
    {
        if (gunLevel == 5)
        {
            //散弹枪
            Instantiate(bullets[bulletIndex], transform.position
                           , Quaternion.Euler(0, 0, (float)(transform.rotation.eulerAngles.z - curInaccuracy)));
            Instantiate(bullets[bulletIndex], transform.position
                           , Quaternion.Euler(0, 0, (float)(transform.rotation.eulerAngles.z - 0.5f * curInaccuracy)));
            Instantiate(bullets[bulletIndex], transform.position
        ...
        else
        {
            Instantiate(bullets[bulletIndex], transform.position
                            , Quaternion.Euler(0, 0, transform.rotation.eulerAngles.z +
                            UnityEngine.Random.Range((float)-curInaccuracy, (float)curInaccuracy)));
        }
    }
    /// <summary>
    /// 监听玩家输入
