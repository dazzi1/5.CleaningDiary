...
public class Player : MonoBehaviour
{
    ...
    public GameObject explosion;//地雷特效
    public GameObject deadBloodGo;//延时死亡
    ...
    public PlayShooting playerShooting;
    ...
    private void PlayerMove() {
        ...
        {
            playerRig.AddForce(new Vector2(0, speed / (float)(1 + 0.1f * playerShooting.weight)) * 70 * Time.deltaTime);
            moveAngle = 0;
            isMoving = true;
        }
        ...
        {
            playerRig.AddForce(new Vector2(-speed / (float)(1 + 0.1f * playerShooting.weight), 0) * 70 * Time.deltaTime);
            moveAngle = 90;
            isMoving = true;
        }
...
#elif UNITY_ANDROID
        playerRig.AddForce(new Vector2(GameManager.Instance.inputValue.x * speed / (float)(1 + 0.1f * playerShooting.weight) * 70 * Time.deltaTime
                                     , GameManager.Instance.inputValue.y * speed / (float)(1 + 0.1f * playerShooting.weight) * 70 * Time.deltaTime));
    ...
    private void RegenHP() {
        if (delayTimer <= 0 && HP < maxHP)
        {
            ...
            delayTimer -= Time.deltaTime;//计时减少
        }
    }
    /// <summary>
    /// 道具
